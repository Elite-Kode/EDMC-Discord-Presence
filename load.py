...
# From discrod-rpc.h
#
discord_rpc = ctypes.cdll.LoadLibrary(join(dirname(
    __file__), platform == 'darwin' and 'libdiscord-rpc.dylib' or 'discord-rpc.dll'))
...
DisconnectedProc = ctypes.CFUNCTYPE(
    None, ctypes.c_int, ctypes.c_char_p)  # errorCode, message
ErroredProc = ctypes.CFUNCTYPE(
    None, ctypes.c_int, ctypes.c_char_p)		# errorCode, message
...
Discord_Initialize.argtypes = [ctypes.c_char_p, ctypes.POINTER(
    DiscordEventHandlers), ctypes.c_int, ctypes.c_char_p]  # applicationId, handlers, autoRegister, optionalSteamId
